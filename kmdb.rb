...
#Corresponding .rb migration files created for each table
...
new_role = Role.new
new_role["movie_id"] = movie["id"]
new_role["actor_id"] = (Actor.find_by({"name" => "Michael Cain"})) ["id"]
...
for each in movies
    ...
    rating = each["rated"]
    studio = Studio.find_by({"id" => each["studio_id"] })
